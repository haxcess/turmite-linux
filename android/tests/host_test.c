...
int main(void)
{
    ...
    CHECK(!android_engine_create(0, 48, 1));
    CHECK(!android_engine_create(961, 48, 1));
    CHECK(!android_engine_frame(NULL, pixels, 64 * 48));
    android_engine_destroy(NULL);
    AndroidEngine *survivor = android_engine_create(64, 48, 123);
    CHECK(survivor);
    for (unsigned cycle = 0; cycle < 30; ++cycle) {
        AndroidEngine *engine = android_engine_create(64, 48, cycle + 1);
        CHECK(engine);
