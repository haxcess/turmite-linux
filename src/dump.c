                        ...
                        const Scheduler *scheduler, const Lfsr32 *rng, double age, double now)
{
    ...
    for (size_t i = 0; i < capture->ant_slots; ++i) {
        ...
        out->id = ant->id;
        out->enabled = atomic_load_explicit(&ant->enabled, memory_order_relaxed);
        out->clobbered = atomic_load_explicit(&ant->clobbered, memory_order_relaxed);
        out->expired = atomic_load_explicit(&ant->expired, memory_order_relaxed);
        out->draining = atomic_load_explicit(&ant->draining, memory_order_relaxed);
        ...
        out->tokens = atomic_load_explicit(&ant->sched.tokens, memory_order_relaxed);
        out->token_rate = atomic_load_explicit(&ant->sched.token_rate, memory_order_relaxed);
        out->token_capacity = atomic_load_explicit(&ant->sched.token_capacity, memory_order_relaxed);
        out->weight = atomic_load_explicit(&ant->sched.weight, memory_order_relaxed);
