...
int main(void)
{
    ...
    for(unsigned i=0;i<20;++i) {
        ...
        if(i%4==0) (void)scheduler_double_population(&scheduler,&world,&rng,now_us());
        if(i%4==2) (void)scheduler_begin_halving(&scheduler,8);
    ...
    printf("mutation stress ok: 8 workers, cloning/halving/capture, %llu collisions, %llu retained mutations\n",
